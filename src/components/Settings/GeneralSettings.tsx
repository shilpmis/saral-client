...
import { Building2, Mail, Phone, MapPin, Crown } from 'lucide-react'
...
export default function GeneralSettings() {
  ...
  function onSubmitBasicSchoolData(values: z.infer<typeof basicSchoolDataSchema>) {
    setLoading(prev => ({ ...prev, basicSchoolData: true }))
    // Simulate API call
    setTimeout(() => {
      setLoading(prev => ({ ...prev, basicSchoolData: false }))
      // toast({
      //   title: "Basic school data updated",
      //   description: "Your school's basic information has been updated successfully.",
      // })
      console.log(values)
    }, 1000)
  }
          ...
          <form onSubmit={basicSchoolDataForm.handleSubmit(onSubmitBasicSchoolData)}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Enter short-key/username for school" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <CardFooter>
              <Button type="submit" disabled={loading.basicSchoolData}>
                {loading.basicSchoolData ? "Saving..." : "Save Basic School Data"}
              </Button>
            </CardFooter>
          </form>
        </Form>
