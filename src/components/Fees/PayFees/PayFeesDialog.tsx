...
//         // Add existing carry forward amount if enabled
//         if (values.use_carry_forward && Number(installment.carry_forward_amount || 0) > 0) {
...
//           // If we have additional discount to apply
//           appliedConcessions = []
...
//             // Include existing concessions
//             appliedConcessions = [...installment.applied_concession]
//           }

//           // Now add new concessions based on available student concessions
...
//         // If this is a carry-forward payment for an already paid installment
...
//                                 value={carryForwardAmounts[installment.id] || ""}
//                                 onChange={(e) => updateCarryForwardAmount(installment.id, e.target.value)}
//                                 className="w-full"
//                                 min="0"
  ...
  const handleSubmit = async (values: z.infer<typeof feePaymentSchema>) => {
    try {
      ...
      const installmentPayments: FeePaymentRequest[] = installments.map((installment : any) => {
        ...
        // Now add new concessions if additional discount is being applied
        if (additionalDiscount > 0 && studentConcessions && studentConcessions.length > 0) {
      ...
      <DialogContent className="sm:max-w-[600px] h-[80vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    // Update the payment mode select options to include Cheque
                    <SelectContent>
                      <SelectItem value="Cash">{t("cash")}</SelectItem>
                      <SelectItem value="Online">{t("online")}</SelectItem>
                      <SelectItem value="Bank Transfer">{t("bank_transfer")}</SelectItem>
            ...
            // Add a special section for carry-forward payments in the dialog
            {installments.some(
              (installment) => installment.is_paid && Number(installment.carry_forward_amount || 0) > 0,
            ) && (
              <Card className="mt-4">
