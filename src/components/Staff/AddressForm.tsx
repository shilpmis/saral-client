...
export function AddressForm({ onSubmit, onPrevious, defaultValues }: AddressFormProps) {
  ...
  const [state, setState] = useState<{ state_id: string; state_name: string; country_id: string }[]>([]);
  const [stateId, setStateId] = useState('');
  const [city, setCity] = useState<{ city_id: string; city_name: string; state_id: string }[]>([]);
...
useEffect(()=>{
  ...
  const city_According_To_State = indiaStatesCities.filter(city => city.state_id === stateId); //here city fetch according to particular state through the stateId
  if(city_According_To_State.length !== 0){
    setCity(city_According_To_State); 
  } 
  else{
setCity([]);
  }      
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={(e)=>{
                  ...
                  <SelectContent>
                    {state.map((stateGet:any) => (
                      <SelectItem key={stateGet.state_id} value={stateGet.state_id}>
                        {stateGet.state_name}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    {city.length === 0 ?<SelectItem value={city.length.toString()}>{"first you select the state"}</SelectItem> : city.map((cityGet:any) => (
                      <SelectItem key={cityGet.city_id} value={cityGet.city_id}>
                        {cityGet.city_name}
                      </SelectItem>
                    ))}
                  </SelectContent>
