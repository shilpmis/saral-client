...
export default function StaffTable({
  ...
  const [currentVisibleData, setCurrentVisibleData] =
    useState<Staff[]>(staffList);

  const perPageData = 6;
  const totalPages = Math.round((staffList.length + 1) / perPageData);

  const paginateData = (page: number): Staff[] => {
    const newVisibleDataSet = staffList.slice(
      Math.max(0, page - 1) * perPageData + 0,
      page * perPageData + 1
    );
...
//     //   const [currentVisibleData_active_inactive, setCurrentVisibleData_active_inactive] = useState<Staff[]>(active_inactive_staff);
...
//         return newVisibleDataSet;
//       }
//     //   const paginateData_active_inactive = (page: number): Staff[] => {
//     //     const newVisibleDataSet_active_inactive = active_inactive_staff.slice((Math.max(0, page - 1)) * perPageData + 1, (page) * perPageData + 1);
//     //     return newVisibleDataSet_active_inactive;
//     //   }

//       const onPageChange = (upadatedPage: number) => {
//         setCurrentVisibleData(paginateData(upadatedPage))
//         // setCurrentVisibleData_active_inactive(paginateData_active_inactive(upadatedPage))
//         setCurrentPage(upadatedPage);
//       }
