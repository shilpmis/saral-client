...
export default function StudentFeesStatus() {
  ...
  const { data : studeneFeesStatus, isLoading, error } = useGetStudentFeesDetailsQuery({ student_id: studentId  , academic_session_id : CurrentAcademicSessionForSchool!.id })
  ...
  return studeneFeesStatus && <FeesStatusContent studeneFeesStatus={(studeneFeesStatus as StudentFeeDetails)} />
}
...
function FeesStatusContent({ studeneFeesStatus }: { studeneFeesStatus: StudentFeeDetails }) {
  ...
  const paymentProgress = hasFeesStatus
    ? Math.min(
        Math.round(
          (Number.parseFloat(feesStatus.paid_amount.toString()) / Number.parseFloat(feesStatus.total_amount || "1")) * 100,
        ),
        100,
      )
    : 0
